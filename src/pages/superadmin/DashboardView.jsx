...
export default function DashboardView({ metrics, loadingMet, onRefreshMetrics, T: propT }) {
  ...
  const KpiCard = ({ label, value, sub, color, icon: Icon, glow }) => (
    <div style={{
      ...
      {glow && <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: (color || T.accent) + "0e", pointerEvents: "none" }} />}
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 6 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 10, color: T.muted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10, fontWeight: 600 }}>{label}</div>
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(22px,4vw,32px)", letterSpacing: 1, color: color || T.text, lineHeight: 1, wordBreak: "break-word" }}>{value}</div>
          {sub && <div style={{ fontSize: 11, color: T.muted, marginTop: 6 }}>{sub}</div>}
        </div>
        {Icon && (
          <div style={{ background: (color || T.accent) + "18", borderRadius: 10, padding: 9, flexShrink: 0, border: `1px solid ${(color || T.accent)}22` }}>
  ...
  const ActivityItem = ({ text, badge, badgeColor, time, sub }) => (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "0.75rem 0", borderBottom: `1px solid ${T.border}` }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
        {badge && <span style={{ background: (badgeColor || T.accent) + "22", color: badgeColor || T.accent, fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 20, letterSpacing: 0.5 }}>{badge}</span>}
    ...
    <div>
      ...
      {loadingMet ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem" }}>
          {[...Array(6)].map((_, i) => (
            <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "1.25rem", height: 90, opacity: 0.5 }} />
          ))}
        </div>
      ) : (
        <>
              ...
              {growthData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={growthData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="revenue" name="Receita" stroke={T.accent} strokeWidth={2.5} dot={{ fill: T.accent, r: 4, strokeWidth: 0 }} activeDot={{ r: 6, strokeWidth: 0 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
              ...
              {growthData.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={growthData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                    ...
                    <Bar dataKey="new_carwashes" name="Cadastros" fill={T.accent} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center", color: T.muted, fontSize: 13 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.25rem" }}>
            ...
            <ChartCard title="Atividade Recente" sub="Resumo dos eventos mais importantes">
              {(m.recent_activity && m.recent_activity.length > 0) ? (
                ...
                <>
                  {totalActive > 0 ? (
                    <ActivityItem text={`${totalActive} lava rápido${totalActive > 1 ? "s" : ""} com assinatura ativa`} badge="ATIVO" badgeColor={T.success} time="agora" />
                  ) : null}
                  {+(m.courtesy_carwashes || 0) > 0 ? (
                    <ActivityItem text={`${m.courtesy_carwashes} acesso${m.courtesy_carwashes > 1 ? "s" : ""} cortesia concedido${m.courtesy_carwashes > 1 ? "s" : ""}`} badge="CORTESIA" badgeColor={T.accent} time="recente" />
                  ) : null}
                  {+(m.total_carwashes || 0) === 0 && (
                    <div style={{ color: T.muted, fontSize: 13, textAlign: "center", padding: "2rem 0" }}>Nenhuma atividade recente</div>
                  )}
                </>
              )}
            </ChartCard>
